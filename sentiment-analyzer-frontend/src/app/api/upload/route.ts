...
export async function POST(req: Request) {
    try {
        ...
        if (env.AWS_REGION === "local") {
            ...
            const key = `local-uploads/${id}${fileType}`;
            ...
            // Return mock S3 URL for local development
            return NextResponse.json({
                url: `http://localhost:3000/api/mock-upload/${id}`,
                fileId: id,
                fileType,
                key,
            });
        ...
        // Original AWS S3 code for production
